...
`
	...
	mirrorExample = templates.Examples(`
		...
		# Delete all ImageDigestMirrorSet generated by oc adm catalog mirror
...
func NewMirrorCatalog(f kcmdutil.Factory, streams genericclioptions.IOStreams) *cobra.Command {
	...
	flags.MarkDeprecated("icsp-scope", "support for it will be removed in a future release.")
	flags.MarkDeprecated("max-icsp-size", "support for it will be removed in a future release.")
	return cmd
}
...
func getRegistryMapping(out io.Writer, icspScope string, mapping map[imagesource.TypedImageReference]imagesource.TypedImageReference) map[string]string {
	...
	for k, v := range mapping {
		if len(v.Ref.ID) == 0 {
			fmt.Fprintf(out, "no digest mapping available for %s, skip writing to ImageDigestMirrorSet\n", k)
			continue
		}
...
func WriteManifests(out io.Writer, source, dest imagesource.TypedImageReference, dir, icspScope string, maxICSPSize int, mapping map[imagesource.TypedImageReference]imagesource.TypedImageReference) error {
	...
	if dest.Type != imagesource.DestinationFile {
		...
		if err := ioutil.WriteFile(filepath.Join(dir, "imageDigestMirrorSet.yaml"), aggregateICSPs(icsps), os.ModePerm); err != nil {
...
func generateICSP(out io.Writer, name string, byteLimit int, registryMapping map[string]string) ([]byte, error) {
	...
	for key := range registryMapping {
		repositoryDigestMirror := apicfgv1.ImageDigestMirrors{
			Source:  key,
			Mirrors: []apicfgv1.ImageMirror{apicfgv1.ImageMirror(registryMapping[key])},
		}
		icsp.Spec.ImageDigestMirrors = append(icsp.Spec.ImageDigestMirrors, repositoryDigestMirror)
		...
		if len(y) > byteLimit {
			if lenMirrors := len(icsp.Spec.ImageDigestMirrors); lenMirrors > 1 {
				icsp.Spec.ImageDigestMirrors = icsp.Spec.ImageDigestMirrors[:lenMirrors-1]
				break
			}
			return nil, fmt.Errorf("unable to add mirror %v to ImageDigestMirrorSet with the max size set to %d", repositoryDigestMirror, byteLimit)
		}
